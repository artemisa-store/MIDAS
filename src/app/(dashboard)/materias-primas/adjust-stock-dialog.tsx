...
export function AdjustStockDialog({
  ...
  const diff = !isNaN(real) ? real - material.stock : 0
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Stock ajustado", {
        description: `${material.name}: ${material.stock} → ${real} (${diff > 0 ? "+" : ""}${diff})`,
      })
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center justify-center gap-4 py-4 bg-muted rounded-xl">
            <div className="text-center">
              ...
              <p className="text-2xl font-bold tabular-nums">{material.stock}</p>
            </div>
          ...
          <div className="space-y-1.5">
            <Label htmlFor="rm-real-stock">Stock real (conteo físico)</Label>
            <Input
              id="rm-real-stock"
              type="number"
              min="0"
          ...
          {/* Razón */}
          <div className="space-y-1.5">
            <Label htmlFor="rm-reason">Razón del ajuste *</Label>
            <Textarea
              id="rm-reason"
